...
export default function CreateListing() {
    ...
    const handleImageSubmit = (e) => {
        if(files.length > 0 && files.length + formData.imageUrls.length < 7){
            ...
            Promise.all(promises).then((urls) =>{
                ...
                setImageUploadError('Carregamento de imagem falhou (2 mb max por imagem)');
                setUploading(false);
            });
        }else{
            setImageUploadError('Você só pode carregar 6 imagens');
            setUploading(false);
        }
    };
        ...
        return new Promise((resolve, reject) =>{
            ...
            uploadTask.on(
                ...
                (snapshot)=>{
                    ...
                    console.log(`Carregamento em ${progress}% feito!`);
                },
                (error)=>{
                    reject(error);
                },
    ...
    const handleSubmit = async(e) =>{
        ...
        try {
            if(formData.imageUrls.length < 1 ) return setError('Você precisa carregar pelo menos uma imagem!');
            if(+formData.regularPrice < +formData.discountPrice) return setError('O preço com desconto deve ser inferior ao preço normal');
            setLoading(true);
            setError(false);
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto'>
        <h1 className='text-3xl font-semibold text-center my-7'>Editar Listagem</h1>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
            ...
            <div className='flex flex-col flex-1 gap-4'>
                ...
                <div className='flex gap-4'>
                    ...
                    <button type='button' disabled={uploading} onClick={handleImageSubmit} className='p-3 text-green-700 border
                     border-green-700 rounded uppercase 
                     hover:shadow-lg disabled:opacity-80'>{uploading ? 'Carregando...' : 'Carregar'}</button>
                </div>
...

                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) =>(
                        <div key={url} className='flex justify-between p-3 border items-center'>
                            ...
                            uppercase hover:opacity-75'>Apagar</button>
                        </div>
                    )
                )
                }
                ...
                <button disabled={loading || uploading} className='p-3 bg-slate-700 text-white rounded-lg
                    ...
                    disabled:opacity-80'>{loading ? 'Editando...' : 'Editar Listagem'}
                </button>
